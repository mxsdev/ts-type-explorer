...
abstract class TypeTreeItem extends vscode.TreeItem {
    ...
    getSignatureChildren(signature: SignatureInfo): TypeNode[] {
        return [
            ...
            ...signature.returnType ? [ this.createChildTypeNode(signature.returnType, { purpose: 'return' }) ] : [],
        ]
    }
}
        ...
        switch(kind) {
            ...
            case "interface": {
                ...
                return [ 
                    ...
                    ...(constructSignatures && constructSignatures.length > 0) ? [toTreeNodeArgs({ kind: 'function', id: -1, signatures: constructSignatures }, { purpose: 'class_constructor' })] : [],
                    ...properties.map(toTreeNode),
                ]
            }
...
function generateTypeNodeMeta(info: ResolvedTypeInfo, dimension: number, {purpose, optional, name: forceName }: TypeNodeArgs = {}) {
    ...
    function getAliasDescription(): string|undefined {
        ...
        switch(info.kind) {
            ...
            case "class": {
                return info.classSymbol?.name ?? defaultValue
            }
